    ...
    public static class Lang
    {
        ...
        public static string S(string s)
        {
            ...
            string[,] rules = {
                ...
                { "该目录下的进度档（progress-*.json）：", "Progress saves (progress-*.json) in that folder:" },
                { "（一个都没有）", "(none found)" },
                { "（存档目录不存在）", "(save folder not found)" },
                { "（无法读取存档目录）", "(cannot read the save folder)" },
                { "说明：progress-*.json 是游戏的进度档，游戏在完成开局、第一次存档之后才会生成。", "Note: progress-*.json are the game's progress saves, created at the first save after the intro." },
                { "· 刚安装游戏或还没开局：先进游戏玩到第一次自动存档，再来读取修改", "· New install / no run yet: play until the first auto-save, then edit" },
                { "· 存档不在上面这个目录：点顶部「浏览…」选择正确的 saves 目录再读取", "· Saves elsewhere: click Browse… above to pick the right saves folder" },
                { "· 想确认目录里有没有档：看上面的列表，或到游戏里手动存一个进度档", "· To check: see the list above, or make a manual save in-game" },
                { "未找到进度档（progress-*.json）：进游戏完成开局、第一次存档后再读取；目录不对可点「浏览…」修改。", "No progress saves (progress-*.json): play to the first save after the intro, then load; use Browse… if the folder is wrong." },
