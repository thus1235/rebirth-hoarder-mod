...
__rhEcoModBind9 = (function () {
  ...
  try {
    ...
    // ===== 任意物品 / 装备添加（window.__RH_ITEM__） =====
    try {
      var itemTable = (typeof __RH_ITEMS__ === 'object' && __RH_ITEMS__) ? __RH_ITEMS__ : null;
      var itemListCache = null;
      window.__RH_ITEM__ = {
        ready: function () { return !!ITEMS; },
        // 全部物品：{id,name,type,rarity}，按中文名排序
        list: function () {
          ...
          for (var id in itemTable) {
            ...
            out.push({
              id: id,
              name: String(d.name || d.displayName || d.label || id),
              type: String(d.type || d.category || d.itemType || ''),
              rarity: String(d.rarity || d.tier || '')
            });
          }
          try { out.sort(function (a, b) { return a.name.localeCompare(b.name, 'zh'); }); } catch (e2) {}
          itemListCache = out;
          return out;
        },
        // 添加普通物品（堆叠）到背包/仓库；返回数量
        add: function (defId, qty, toStash) {
          var setGS = window.__RH_UE__;
          if (typeof setGS !== 'function' || !defId) return 0;
          var n = Math.max(1, Math.floor(Number(qty) || 1));
          setGS(function (prev) {
            if (!prev) return prev;
            if (toStash) {
              var st = (prev.stash || []).map(function (x) { return Object.assign({}, x); });
              pushItem(st, defId, n);
              return Object.assign({}, prev, { stash: st });
            }
            var inv = (prev.inventory || []).map(function (x) { return Object.assign({}, x); });
            pushItem(inv, defId, n);
            return Object.assign({}, prev, { inventory: inv });
          });
          return n;
        },
        // 添加装备实例（带等级/强化/改造/稀有度），放入背包
        addGear: function (defId, opt) {
          var setGS = window.__RH_UE__;
          if (typeof setGS !== 'function' || !defId) return null;
          opt = opt || {};
          var def = itemTable && itemTable[defId];
          var level = Math.max(1, Math.floor(Number(opt.level) || 1));
          var enh = Math.max(0, Math.floor(Number(opt.enhance) || 0));
          var upg = Math.max(0, Math.floor(Number(opt.upgrade) || 0));
          var rarity = opt.rarity || (def && (def.rarity || def.tier)) || 'epic';
          var bonus = rarity === 'artifact' ? 0.6 : rarity === 'legendary' ? 0.4 : rarity === 'epic' ? 0.2 : 0;
          var scale = Math.min(3, Math.max(0.5, 0.8 + level * 0.02 + enh * 0.04 + upg * 0.02 + bonus));
          var inst = {
            instanceId: 'rhgear_' + Date.now() + '_' + Math.floor(Math.random() * 99999),
            defId: defId,
            quantity: 1,
            x: -1, y: -1, rotation: 0,
            rarity: rarity,
            ...
            enhanceLevel: enh,
            upgradeLevel: upg,
            qualityTier: opt.qualityTier || 'mid',
            affixes: [],
            affixName: (def && (def.name || def.displayName)) || defId,
            statScale: scale,
            durability: 9999,
            locked: false,
            selfRepair: false
          };
          if (rarity === 'artifact') inst.artifactSourceBaseId = defId;
          setGS(function (prev) {
            if (!prev) return prev;
            var inv = (prev.inventory || []).map(function (x) { return Object.assign({}, x); });
            inv.push(inst);
      ...
      try { window.__RH_DIAG_ERR__ = String(eItem && eItem.message || eItem); } catch (e3) {}
    }
  } catch (e) {
